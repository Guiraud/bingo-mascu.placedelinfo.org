    ...
    form.addEventListener('submit', async e => {
      ...
      try {
        const res = await fetch(apiUrl('/api/argumentaires'), {
          ...
          body: JSON.stringify({ phrase, argumentaire: arg, sources, turnstile_token: (document.querySelector('[name="cf-turnstile-response"]') || {}).value || '' })
        });
        if (!res.ok) {
        ...
        alert("Impossible d'enregistrer dans la base. Voir la console pour les détails.");
      } finally {
