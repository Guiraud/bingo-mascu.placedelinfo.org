  ...
  const PHENOMENA_BY_ID = new Map(PATRIARCHAL_PHENOMENA.map(item => [item.id, item]));
    ...
    function setDetails(html, { persist = true } = {}) {
      ...
      if (persist) lastDetailHTML = html;
    }
    ...
    function restoreLastDetails() {
      if (lastDetailHTML) detailsEl.innerHTML = lastDetailHTML;
      else renderDefaultDetails();
    }
    ...
    function addGlitterFrame(slot) {
      ...
      for (let i = 0; i < STAR_COUNT; i++) {
        ...
        switch (side) {
          case 0: star.style.top = '-8px'; star.style.left = `${offset}%`; break;
          case 1: star.style.right = '-8px'; star.style.top = `${offset}%`; break;
          case 2: star.style.bottom = '-8px'; star.style.left = `${offset}%`; break;
          default: star.style.left = '-8px'; star.style.top = `${offset}%`; break;
        }
    ...
    function buildBoard() {
      ...
      if (detailConfig) {
        const detailSlotElement = document.createElement('div');
      ...
      for (let r = 0; r < size; r++) {
        for (let c = 0; c < size; c++) {
          if (detailSkip && detailSkip.has(`${r}-${c}`)) continue;
    ...
    function toggleCell(cell) {
      ...
      if (cell.dataset.cellType === 'phenomenon') {
        ...
        if (phenomenon) { renderPhenomenonDetails(phenomenon); return; }
      }
      showArgumentaire(cell.textContent.trim());
    }

    function showArgumentaire(phrase) {
      const arg = ARGUMENTAIRES[phrase] || "Pas encore d'argumentaire détaillé pour cette phrase.";
      const sourcesHtml = renderSourcesHtml(ARG_SOURCES[phrase] || []);
    ...
    function checkBingo() {
      ...
      for (const line of lines) {
        ...
        for (const [r, c] of line) {
          const cell = boardMatrix[r]?.[c] || null;
          if (!cell || !cell.classList.contains('marked')) { full = false; break; }
          cellsInLine.push(cell);
        }
        if (full) { bingoCount++; cellsInLine.forEach(cell => cell.classList.add('bingo')); }
      }
    ...
    function updateStats() {
      ...
      if (lastBingoText) parts.push(lastBingoText);
